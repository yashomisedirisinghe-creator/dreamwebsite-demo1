...
*/
    ...
    const simulateBtn = document.getElementById('simulate-btn');
    const forgeUi = document.querySelector('.forge-container');

    let characterSelected = false;
    let envSelected = false;

    // Check if both are selected to reveal the button
    const checkSelections = () => {
        if (characterSelected && envSelected && simulateBtn) {
            simulateBtn.classList.remove('hidden');
            simulateBtn.style.opacity = '1';
            simulateBtn.style.pointerEvents = 'auto';
            simulateBtn.style.transform = 'translateY(0)';
    ...
    if (characterCards.length > 0) {
        ...
        // Handle the final transition
        if (simulateBtn) {
            simulateBtn.addEventListener('click', () => {
                // Ensure the forgeUi element exists before applying the class
                if(forgeUi) forgeUi.classList.add('fade-to-black');
                setTimeout(() => {
                    window.location.href = './simulation.html';
                }, 1500);
            });
    ...
    if (simBody && simControls) {
		...
		// --- Cinematic Color Grading Logic ---
        const btnNormal = document.getElementById('grade-normal');
        const btnSepia = document.getElementById('grade-sepia');
        const btnTeal = document.getElementById('grade-teal');

        if (btnNormal && btnSepia && btnTeal && dreamVideo) {
            btnNormal.addEventListener('click', () => {
                dreamVideo.className = 'simulation-visuals'; // Resets to default
            });
            
            btnSepia.addEventListener('click', () => {
                dreamVideo.className = 'simulation-visuals grade-sepia'; // Applies Historical
            });
            
            btnTeal.addEventListener('click', () => {
                dreamVideo.className = 'simulation-visuals grade-teal'; // Applies Suspense
            });
        }
...
if (saveBtn) {
    saveBtn.addEventListener('click', () => {
        ...
        // Provide visual feedback that the simulation was saved
        saveBtn.textContent = 'Echo Archived...';
        saveBtn.style.background = 'rgba(107, 138, 240, 0.3)';
        saveBtn.style.borderColor = 'var(--neon-accent)';
        saveBtn.style.boxShadow = '0 0 15px var(--neon-accent)';
